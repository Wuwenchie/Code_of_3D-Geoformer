...
for i, time in enumerate(times):
    data = sea_temp.sel(time=time, lat=lat, method='nearest')[lead_time]  # lead_time=0
    ...
    # # 創建用於風應力繪圖的網格（僅在最淺層）
    # lon_quiver, lat_quiver = np.meshgrid(lon[::5], lat_new[::5])
    # U = taux_data.values[::5, ::5]  # 經度方向風應力
    # V = tauy_data.values[::5, ::5]  # 緯度方向風應力
    ...
    #                scale=scale, color='black', width=0.007)
